...
var require_picomatch = __commonJS((exports2, module2) => {
  ...
  picomatch.parse = (pattern, options) => {
    ...
    return parse2(pattern, __assign(__assign({}, options), {fastpaths: false}));
  };
...
function createDescriptor(filename, source, root, isProduction) {
  ...
  const normalizedPath = (0, import_slash.default)(import_path2.default.normalize(import_path2.default.relative(root, filename)));
...
function getDescriptor(filename, errorOnMissing = true) {
  ...
  if (errorOnMissing) {
    throw new Error(`${filename} has no corresponding SFC entry in the cache. This is a @vitejs/plugin-vue internal error, please open an issue.`);
  }
}
...
// src/script.ts
var import_compiler_sfc3 = __toModule(require("@vue/compiler-sfc"));

// src/template.ts
var import_path3 = __toModule(require("path"));
var import_slash2 = __toModule(require_slash());
...
function compile(code, descriptor, options, pluginContext, ssr) {
  ...
  const result = (0, import_compiler_sfc2.compileTemplate)(__assign(__assign({}, resolveTemplateCompilerOptions(descriptor, options, ssr)), {
    source: code
  }));
...
function resolveTemplateCompilerOptions(descriptor, options, ssr) {
  ...
  if (options.devServer) {
    if (filename.startsWith(options.root)) {
      assetUrlOptions = {
        base: options.devServer.config.base + (0, import_slash2.default)(import_path3.default.relative(options.root, import_path3.default.dirname(filename)))
      };
    }
  } else {
    assetUrlOptions = {
  ...
  if (transformAssetUrls && typeof transformAssetUrls === "object") {
    if (Object.keys(transformAssetUrls).some((key) => Array.isArray(transformAssetUrls[key]))) {
      transformAssetUrls = __assign(__assign({}, assetUrlOptions), {
        tags: transformAssetUrls
      });
    } else {
      transformAssetUrls = __assign(__assign({}, transformAssetUrls), assetUrlOptions);
    }
  } else {
  ...
  if (block.lang === "pug") {
    preprocessOptions = __assign({
      doctype: "html"
    }, preprocessOptions);
  }
  return __assign(__assign({}, options.template), {
    id,
    filename,
    scoped: hasScoped,
    ...
    preprocessOptions,
    compilerOptions: __assign(__assign({}, (_c = options.template) == null ? void 0 : _c.compilerOptions), {
...
function resolveScript(descriptor, options, ssr) {
  ...
  let resolved = null;
  resolved = (0, import_compiler_sfc3.compileScript)(descriptor, __assign(__assign({}, options.script), {
    id: descriptor.id,
    isProd: options.isProduction,
...
var import_compiler_sfc4 = __toModule(require("@vue/compiler-sfc"));
...
async function handleHotUpdate({
  ...
  const {descriptor} = createDescriptor(file, content, server.config.root, false);
  let needRerender = false;
  const affectedModules = new Set();
  const mainModule = modules.find((m) => !/type=/.test(m.url) || /type=script/.test(m.url));
  const templateModule = modules.find((m) => /type=template/.test(m.url));
  if (!isEqualBlock(descriptor.script, prevDescriptor.script) || !isEqualBlock(descriptor.scriptSetup, prevDescriptor.scriptSetup)) {
...
function isOnlyTemplateChanged(prev, next) {
  return isEqualBlock(prev.script, next.script) && isEqualBlock(prev.scriptSetup, next.scriptSetup) && prev.styles.length === next.styles.length && prev.styles.every((s, i) => isEqualBlock(s, next.styles[i])) && prev.customBlocks.length === next.customBlocks.length && prev.customBlocks.every((s, i) => isEqualBlock(s, next.customBlocks[i]));
}
...
async function transformMain(code, filename, options, pluginContext, ssr) {
  ...
  const {descriptor, errors} = createDescriptor(filename, code, root, isProduction);
  if (errors.length) {
  ...
  const useInlineTemplate = !devServer && descriptor.scriptSetup && !(descriptor.template && descriptor.template.src);
  ...
  let renderReplace = "";
  if (hasTemplateImport) {
    renderReplace = ssr ? `_sfc_main.ssrRender = _sfc_ssrRender` : `_sfc_main.render = _sfc_render`;
  } else {
    if (prevDescriptor && !isEqualBlock(descriptor.template, prevDescriptor.template)) {
      renderReplace = ssr ? `_sfc_main.ssrRender = () => {}` : `_sfc_main.render = () => {}`;
    }
  }
  ...
  if (ssr) {
    output.push(`import { useSSRContext } from 'vue'`, `const _sfc_setup = _sfc_main.setup`, `_sfc_main.setup = (props, ctx) => {`, `  const ssrContext = useSSRContext()`, `  ;(ssrContext.modules || (ssrContext.modules = new Set())).add(${JSON.stringify(filename)})`, `  return _sfc_setup ? _sfc_setup(props, ctx) : undefined`, `}`);
  }
  let resolvedMap = map;
  if (map && templateMap) {
...
const _sfc_main = ${classMatch[1]}`;
        if (/export\s+default/.test(scriptCode)) {
          scriptCode = (0, import_compiler_sfc4.rewriteDefault)(script.content, `_sfc_main`);
        }
      } else {
        scriptCode = (0, import_compiler_sfc4.rewriteDefault)(script.content, `_sfc_main`);
      }
...
// src/style.ts
var import_compiler_sfc5 = __toModule(require("@vue/compiler-sfc"));
async function transformStyle(code, descriptor, index, options, pluginContext) {
  const block = descriptor.styles[index];
  const result = await (0, import_compiler_sfc5.compileStyleAsync)(__assign(__assign({}, options.style), {
...
try {
  ...
  throw new Error("@vitejs/plugin-vue requires @vue/compiler-sfc to be present in the dependency tree.");
}
